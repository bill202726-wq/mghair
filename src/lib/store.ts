...
export const subscribeToSubmissions = (callback: (data: FormState[]) => void) => {
  try {
    ...
    return onSnapshot(q, (snapshot) => {
      ...
      console.warn("Firebase realtime listener error, falling back to local data:", error);
      const data = localStorage.getItem("aapi_submissions");
      if (data) {
        try {
          callback(JSON.parse(data));
        } catch (parseErr) {
          console.error("Error parsing local storage fallback", parseErr);
        }
      }
    });
  } catch (err) {
    console.warn("Unable to start Firestore listener:", err);
    const data = localStorage.getItem("aapi_submissions");
    if (data) {
      try {
        callback(JSON.parse(data));
